...
import type { FrostedModalRegisterPageProps } from '../types'
import '../styles/FrostedModalPage.css'

export const FrostedModalRegisterPage: React.FC<FrostedModalRegisterPageProps> = ({
  onRegister,
  onNavigateToLogin,
  showCaptcha = false,
  showEmailField = true,
        ...
        <div className="yggjs-frosted-card" style={{ ['--card-w' as any]: `${cardSize.width}px`, ['--card-h' as any]: `${cardSize.height}px` } as React.CSSProperties}>
          {(logo || title) && (
            <div style={{ textAlign: 'center', marginBottom: 12 }}>
              ...
              {subtitle && <p style={{ marginTop: 6, color: '#cbd5e1', textAlign: 'center' }}>{subtitle}</p>}
            </div>
          )}
          ...
          <form onSubmit={handleSubmit}>
            ...
            <div className="yggjs-frosted-field">
              <input
                ...
                className="yggjs-frosted-input"
                disabled={disabled || loading || isSubmitting}
                aria-label={usernameLabel}
                autoComplete="username"
              />
              {touched.username && errors.username && <div className="yggjs-frosted-error" role="alert">{errors.username}</div>}
            </div>
            ...
            {showEmailField && (
              <div className="yggjs-frosted-field">
                <input
                  ...
                  autoComplete="email"
                />
                {touched.email && errors.email && <div className="yggjs-frosted-error" role="alert">{errors.email}</div>}
              </div>
            )}
            ...
            <div className="yggjs-frosted-field">
              ...
              />
              {touched.password && errors.password && <div className="yggjs-frosted-error" role="alert">{errors.password}</div>}
            </div>
            ...
            <div className="yggjs-frosted-field">
              <input
                ...
                autoComplete="new-password"
              />
              {touched.confirmPassword && errors.confirmPassword && <div className="yggjs-frosted-error" role="alert">{errors.confirmPassword}</div>}
            </div>
            ...
            {showCaptcha && (
              <div className="yggjs-frosted-field">
                <CaptchaInput
                  ...
                  inputClassName="yggjs-frosted-input"
                  disabled={disabled || loading || isSubmitting}
                />
                {touched.captcha && errors.captcha && <div className="yggjs-frosted-error" role="alert">{errors.captcha}</div>}
              </div>
            )}
            ...
            {showAgreement && (
              <div className="yggjs-frosted-field">
                ...
                {errors.agreement && <div className="yggjs-frosted-error" role="alert">{errors.agreement}</div>}
              </div>
            )}

            {/* 提交 */}
            <button type="submit" className="yggjs-frosted-button" disabled={disabled || loading || isSubmitting}>
              {loading || isSubmitting ? '注册中...' : submitLabel}
            </button>

            {onNavigateToLogin && (
              <div style={{ textAlign: 'center', marginTop: 12 }}>
                <button type="button" className="yggjs-frosted-button" onClick={onNavigateToLogin} disabled={disabled || loading || isSubmitting}>
                  {loginLinkText}
                </button>
              </div>
            )}
