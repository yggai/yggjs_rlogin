...
import type { IllustrationRegisterPageProps } from '../types'
import '../styles/IllustrationLoginPage.css'
import { DefaultRocketIllustration } from './Illustrations'

export const IllustrationRegisterPage: React.FC<IllustrationRegisterPageProps> = ({
  onRegister,
  onNavigateToLogin,
  showCaptcha = false,
  showEmailField = true,
        ...
        <div className="yggjs-illustration-card">
          {(logo || title) && (
            <div style={{ textAlign: 'center', marginBottom: 20 }}>
              ...
              {subtitle && <p style={{ marginTop: 6, color: '#475569', textAlign: 'center' }}>{subtitle}</p>}
            </div>
          )}
          ...
          <form onSubmit={handleSubmit}>
            ...
            <div className="yggjs-illustration-field">
              ...
              />
              {touched.username && errors.username && <div className="yggjs-illustration-error" role="alert">{errors.username}</div>}
            </div>
            ...
            {showEmailField && (
              <div className="yggjs-illustration-field">
                <input
                  ...
                  autoComplete="email"
                />
                {touched.email && errors.email && <div className="yggjs-illustration-error" role="alert">{errors.email}</div>}
              </div>
            )}
            ...
            <div className="yggjs-illustration-field">
              ...
              />
              {touched.password && errors.password && <div className="yggjs-illustration-error" role="alert">{errors.password}</div>}
            </div>
            ...
            <div className="yggjs-illustration-field">
              <input
                ...
                autoComplete="new-password"
              />
              {touched.confirmPassword && errors.confirmPassword && <div className="yggjs-illustration-error" role="alert">{errors.confirmPassword}</div>}
            </div>
            ...
            {showCaptcha && (
              <div className="yggjs-illustration-field">
                <CaptchaInput
                  ...
                  disabled={disabled || loading || isSubmitting}
                />
                {touched.captcha && errors.captcha && <div className="yggjs-illustration-error" role="alert">{errors.captcha}</div>}
              </div>
            )}
            ...
            {showAgreement && (
              <div className="yggjs-illustration-field">
                ...
                {errors.agreement && <div className="yggjs-illustration-error" role="alert">{errors.agreement}</div>}
              </div>
            )}

            {/* 提交 */}
            <button type="submit" className="yggjs-illustration-button" disabled={disabled || loading || isSubmitting}>
              {loading || isSubmitting ? '注册中...' : submitLabel}
            </button>

            {onNavigateToLogin && (
              <div style={{ textAlign: 'center', marginTop: 12 }}>
                <button type="button" className="yggjs-illustration-button" onClick={onNavigateToLogin} disabled={disabled || loading || isSubmitting}>
                  {loginLinkText}
                </button>
              </div>
            )}
