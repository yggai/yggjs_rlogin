...
import type { SplitBrandRegisterPageProps } from '../types'
import '../styles/SplitBrandPage.css'

export const SplitBrandRegisterPage: React.FC<SplitBrandRegisterPageProps> = ({
  onRegister,
  onNavigateToLogin,
  showCaptcha = false,
  showEmailField = true,
        ...
        <div className="yggjs-splitbrand-card" style={{ ['--brand-color' as any]: brandColor }}>
          {(logo || title) && (
            <div style={{ textAlign: 'center', marginBottom: 16 }}>
              ...
              {subtitle && <p style={{ marginTop: 6, color: '#94a3b8' }}>{subtitle}</p>}
            </div>
          )}
          ...
          <form onSubmit={handleSubmit}>
            ...
            <div className="yggjs-splitbrand-field">
              <input
                ...
                className="yggjs-splitbrand-input"
                disabled={disabled || loading || isSubmitting}
                aria-label={usernameLabel}
                autoComplete="username"
              />
              {touched.username && errors.username && <div className="yggjs-splitbrand-error" role="alert">{errors.username}</div>}
            </div>
            ...
            {showEmailField && (
              <div className="yggjs-splitbrand-field">
                <input
                  ...
                  autoComplete="email"
                />
                {touched.email && errors.email && <div className="yggjs-splitbrand-error" role="alert">{errors.email}</div>}
              </div>
            )}
            ...
            <div className="yggjs-splitbrand-field">
              ...
              />
              {touched.password && errors.password && <div className="yggjs-splitbrand-error" role="alert">{errors.password}</div>}
            </div>
            ...
            <div className="yggjs-splitbrand-field">
              <input
                ...
                autoComplete="new-password"
              />
              {touched.confirmPassword && errors.confirmPassword && <div className="yggjs-splitbrand-error" role="alert">{errors.confirmPassword}</div>}
            </div>
            ...
            {showCaptcha && (
              <div className="yggjs-splitbrand-field">
                <CaptchaInput
                  ...
                  inputClassName="yggjs-splitbrand-input"
                  disabled={disabled || loading || isSubmitting}
                />
                {touched.captcha && errors.captcha && <div className="yggjs-splitbrand-error" role="alert">{errors.captcha}</div>}
              </div>
            )}
            ...
            {showAgreement && (
              <div className="yggjs-splitbrand-field">
                ...
                {errors.agreement && <div className="yggjs-splitbrand-error" role="alert">{errors.agreement}</div>}
              </div>
            )}

            {/* 提交 */}
            <button type="submit" className="yggjs-splitbrand-button" style={{ ['--brand-color' as any]: brandColor }} disabled={disabled || loading || isSubmitting}>
              {loading || isSubmitting ? '注册中...' : submitLabel}
            </button>

            {onNavigateToLogin && (
              <div style={{ textAlign: 'center', marginTop: 12 }}>
                <button type="button" className="yggjs-splitbrand-button" onClick={onNavigateToLogin} disabled={disabled || loading || isSubmitting}>
                  {loginLinkText}
                </button>
              </div>
            )}
