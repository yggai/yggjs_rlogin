...
import type { RetroPixelRegisterPageProps } from '../types'
import '../styles/RetroPixelPage.css'

export const RetroPixelRegisterPage: React.FC<RetroPixelRegisterPageProps> = ({
  onRegister,
  onNavigateToLogin,
  showCaptcha = false,
  showEmailField = true,
      ...
      <div className="yggjs-retro-card">
        {(logo || title) && (
          <div style={{ textAlign: 'center', marginBottom: 10 }}>
            ...
            {subtitle && <p style={{ marginTop: 6, color: 'currentColor', opacity:.9, textAlign:'center' }}>{subtitle}</p>}
          </div>
        )}
        ...
        <form onSubmit={handleSubmit}>
          ...
          <div className="yggjs-retro-field">
            <input
              ...
              className="yggjs-retro-input"
              disabled={disabled || loading || isSubmitting}
              aria-label={usernameLabel}
              autoComplete="username"
            />
            {touched.username && errors.username && <div className="yggjs-retro-error" role="alert">{errors.username}</div>}
          </div>
          ...
          {showEmailField && (
            <div className="yggjs-retro-field">
              <input
                ...
                autoComplete="email"
              />
              {touched.email && errors.email && <div className="yggjs-retro-error" role="alert">{errors.email}</div>}
            </div>
          )}
          ...
          <div className="yggjs-retro-field">
            ...
            />
            {touched.password && errors.password && <div className="yggjs-retro-error" role="alert">{errors.password}</div>}
          </div>
          ...
          <div className="yggjs-retro-field">
            <input
              ...
              autoComplete="new-password"
            />
            {touched.confirmPassword && errors.confirmPassword && <div className="yggjs-retro-error" role="alert">{errors.confirmPassword}</div>}
          </div>
          ...
          {showCaptcha && (
            <div className="yggjs-retro-field">
              <CaptchaInput
                ...
                inputClassName="yggjs-retro-input"
                disabled={disabled || loading || isSubmitting}
              />
              {touched.captcha && errors.captcha && <div className="yggjs-retro-error" role="alert">{errors.captcha}</div>}
            </div>
          )}
          ...
          {showAgreement && (
            <div className="yggjs-retro-field">
              ...
              {errors.agreement && <div className="yggjs-retro-error" role="alert">{errors.agreement}</div>}
            </div>
          )}

          {/* 提交 */}
          <button type="submit" className="yggjs-retro-button" disabled={disabled || loading || isSubmitting}>
            {loading || isSubmitting ? 'SAVING...' : submitLabel}
          </button>

          {onNavigateToLogin && (
            <div style={{ textAlign: 'center', marginTop: 12 }}>
              <button type="button" className="yggjs-retro-button" onClick={onNavigateToLogin} disabled={disabled || loading || isSubmitting}>
                {loginLinkText}
              </button>
            </div>
          )}
